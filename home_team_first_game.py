# In this code, we change the question slightly: how many hockey teams can play their season opener at home?
...
for d in dates:
    ...
    for team in list_of_teams:
        ...
        home_team = df_team[df_team['game_date']==first_game]['home_team'].values

        if home_team == team:
            first_game_at_home += 1
        if win_or_lose == team:
            won_first_game += 1
    ...
    season_opener_home[d] = [days_for_all_teams_to_play,
                             first_game_at_home,
                             len(list_of_teams) - won_first_game,
                             len(list_of_teams),
                             d
...
cols = ['days_to_play', 'first_game_away', 'first_game_home', 'num_teams', 'season']
seasons_df = pd.DataFrame(columns=cols,
                          data=[season_opener_home[d] for d in season_opener_home.keys()]
                          )

seasons_df['theoretical_min'] = seasons_df.apply(lambda x: theoretical_min(x['num_teams'], x['days_to_play']), axis=1)

seasons_df['reality_ratio'] = seasons_df['theoretical_min']/seasons_df['first_game_away']
